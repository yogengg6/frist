...
int CIWOutlookBar::AddRecord(int nType, COpenEBTSRecord* pRecord)
{
	int nFolder = -1;
	...
	switch(nType)
	{
		...
		case 2:
		{
			...
			{
				COpenEBTSField* pField = pRecord->m_arrFields.GetAt(nIndex);
...
void CIWOutlookBar::SetCurFolder(int nFolder, bool bAnimation)
{
	...
	switch(pFolder->lParam)
	{
		...
		case 2:
		{
			...
			COpenEBTSField* pField = (COpenEBTSField*) pDlg->m_ctlList.GetItemData(nSelectedIndex);
		...
		case 17:
		{
			...
			COpenEBTSRecord* pRecord = (COpenEBTSRecord*) pDlg->m_ctlList.GetItemData(nSelectedIndex);
