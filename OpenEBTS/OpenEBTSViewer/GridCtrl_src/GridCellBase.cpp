...
void DrawGradientRect(CDC * pDC, CRect &rect, COLORREF cr1, COLORREF cr2, DWORD dwStyle)
{
#if(WINVER >= 0x0500)
	TRIVERTEX        vert[2] ;
	GRADIENT_RECT    gRect;
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        if (bHiliteFixed)
        {
			...
			DrawGradientRect(pDC, rect, cr1, cr2, GRADIENT_FILL_RECT_V);

        }
        else
        {
   //         CPen pen(PS_SOLID, 1, RGB(0,45,150)), *pOldPen = pDC->GetCurrentPen();

   //         pDC->SelectObject(&pen);
			//pDC->Rectangle(&rect);

			COLORREF cr1 = RGB(203,225,252);
			COLORREF cr2 = RGB(155,188,234);
			DrawGradientRect(pDC, rect, cr1, cr2, GRADIENT_FILL_RECT_V);
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            //  working with CMemDC2 directly.  Instead, don't display image
